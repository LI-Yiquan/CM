...
from fine_tune.encoder import FeatureEncoder, Classifier
...
class CMTrainLoop(TrainLoop):
    def __init__(
        ...
        **kwargs,
        ...
        self.use_official_edm = use_official_edm
        if target_model:
        ...
        self.encoder_optimizer = th.optim.Adam(self.encoder.parameters(), lr=0.1)
    ...
    def run_loop(self):
        ...
        while (
            ...
            print(f"Total memory for gpu 0: {info.total/1024/1024} Mb")
            print(f"Free memory for gpu 0: {info.free/1024/1024} Mb")
            print(f"Used memory for gpu 0: {info.used/1024/1024} Mb")"""
            self.run_step(batch, cond)
            saved = False
            if (
                self.global_step
    ...
    def forward_backward(self, batch, cond):
        ...
        for i in range(0, batch.shape[0], self.microbatch):
            ...
            if self.training_mode == "progdist":
                ...
                if self.use_official_edm:
                    compute_losses = functools.partial(
                        self.diffusion.consistency_losses_official_edm,
                        self.ddp_model,
                        micro,
                        num_scales,
                        target_model=self.target_model,
            ...
            elif self.training_mode == "consistency_contrastive_distillation":
                  compute_losses = functools.partial(
                        self.diffusion.consistency_contrastive_losses,
                        self.ddp_model,
                        self.encoder,
                        self.classifier,
            ...
            if last_batch or not self.use_ddp:
                losses = compute_losses() # first time and second time
            else:
                with self.ddp_model.no_sync():
                    losses = compute_losses()
