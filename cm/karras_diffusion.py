...
from fine_tune.finetune_loss import FineTuneLoss
from fine_tune.encoder import FeatureEncoder
...
class KarrasDenoiser:
    ...
    def consistency_losses_official_edm(
        self,
        model,
        x_start,
        num_scales,
        ...
        def heun_solver(samples, t, next_t, x0):
            ...
            if teacher_model is None:
                ...
                # denoiser = teacher_denoise_fn(x, t)
                denoiser = teacher_model(x, t, augment_labels=None)
            ...
            if teacher_model is None:
                ...
                # denoiser = teacher_denoise_fn(samples, next_t)
                denoiser = teacher_model(samples, next_t, augment_labels=None)
        ...
        def euler_solver(samples, t, next_t, x0):
            ...
            if teacher_model is None:
                ...
                # denoiser = teacher_diffusion(x, t)
                denoiser = teacher_model(x, t, augment_labels=None)
            d = (x - denoiser) / append_dims(t, dims)
            samples = x + d * append_dims(next_t - t, dims)
        ...
        dropout_state = th.get_rng_state()
        # distiller = denoise_fn(x_t, t)
        distiller = model(x_t, t, augment_labels=None) 
        ...
        th.set_rng_state(dropout_state)
        # distiller_target = target_denoise_fn(x_t2, t2)
        distiller_target = target_model(x_t2, t2, augment_labels=None)
        distiller_target = distiller_target.detach()
    ...
    def consistency_contrastive_losses(
        self,
        model,
        encoder,
        classifier,
        ...
        snrs = self.get_snr(t)
        weights = get_weightings(self.weight_schedule, snrs, self.sigma_data)
        ori_distiller = distiller
        ori_distiller_target = distiller_target
        if self.loss_norm == "l1":
            diffs = th.abs(distiller - distiller_target)
            loss = mean_flat(diffs) * weights
        elif self.loss_norm == "l2":
        ...
        mask_indices = th.nonzero(t<=1).squeeze(-1) # drop out when t>1
        ...
        if mask_indices.size()[0] != 0:
            ...
            output1 = ori_distiller[mask_indices]
            output2 = x_start[mask_indices]

            all_features1 = encoder(output1.to(th.device("cuda")))
            all_features2 = encoder(output2.to(th.device("cuda")))
            
            features = th.cat([all_features1,all_features2])
            
            temperature = 0.1
            
            logits, labels = FineTuneLoss.pure_contrastive_loss(features,temperature, th.device("cuda"))
            
            criterion = th.nn.CrossEntropyLoss().to(x_start.device)
            
            contrastive_loss = criterion(logits, labels)/(features.size()[0]/2)
            
            encoder_optimizer.zero_grad()
            contrastive_loss.backward(retain_graph=True)  
            encoder_optimizer.step()  # Update model parameters
                
        terms = {}
        terms["loss"] = loss + 0.1 * contrastive_loss
        
        # terms["pred accuracy"] = pred_acc
...
def karras_sample(
    ...
    if purification == True:
        x_T = corrupted_images
    else:
        x_T = generator.randn(*shape, device=device) * sigma_max

    sample_fn = {
        "heun": sample_heun,
        "heun_purification":sample_heun_purification,
        "dpm": sample_dpm,
        ...
        "multistep_purification":stochastic_iterative_sampler_purification
    }[sampler]
    ...
    elif sampler == "multistep_purification":
        sampler_args = dict(
            ts=ts, t_min=sigma_min, t_max=sigma_max, rho=diffusion.rho, steps=steps,noise_sigma=noise_sigma
        )
    elif sampler == "heun_purification":
        sampler_args = dict(
            s_churn=s_churn, s_tmin=s_tmin, s_tmax=s_tmax, s_noise=s_noise,noise_sigma=noise_sigma
        )
    else:
        sampler_args = {}
...
@th.no_grad()
def stochastic_iterative_sampler_purification(
    distiller,
    x,
    sigmas,
    generator,
    ...
    step_indices = th.arange(steps, dtype=th.float64,device=x.device)
    t_steps = (t_max ** (1 / rho) + step_indices / (steps - 1) * (t_min ** (1 / rho) - t_max ** (1 / rho))) ** rho
    for i in range(len(t_steps)-1):
        if t_steps[i]>=noise_sigma and t_steps[i+1]<noise_sigma:
            if t_steps[i]-noise_sigma > noise_sigma-t_steps[i+1]:
                t = i+1
                break
            else:
                t = i
    ...
    """
    # first add a difference noise to the img
    ...
    x0 = distiller(x, noise_sigma * s_in) # directly use noise_sigma
    next_t = t_min_rho ** rho
    next_t = np.clip(next_t, t_min, t_max)
    x = x0 + generator.randn_like(x) * np.sqrt(next_t**2 - t_min**2)
    """
